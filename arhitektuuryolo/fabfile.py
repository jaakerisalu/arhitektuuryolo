...
DATABASES = {
    'default': {
        ...
        'NAME': 'arhitektuuryolo',
        'USER': 'arhitektuuryolo',
...
def defaults():
    ...
    env.service_name = ["gunicorn-arhitektuuryolo"]
    env.code_dir = '/srv/arhitektuuryolo'
...
def deploy(id=None, silent=False, force=False):
    ...
    package_changed = force or vcs.changed_files(revset, r' arhitektuuryolo/package.json')
    if package_changed:
        print colors.yellow("Will run npm install")

    # See if we have changes in app source or static files
    app_changed = force or vcs.changed_files(revset, [r' arhitektuuryolo/app', r' arhitektuuryolo/static', r' arhitektuuryolo/settings', r'webpack'])
    if app_changed:
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/arhitektuuryolo')
...
def setup_server():
    ...
    # Create database
    sudo('echo "CREATE DATABASE arhitektuuryolo; '
         '      CREATE USER arhitektuuryolo WITH password \'%s\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE arhitektuuryolo to arhitektuuryolo;" '
    ...
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/arhitektuuryolo/settings/local.py', use_sudo=True)

    # Create necessary dirs, with correct permissions
    mkdir_wwwdata('/var/log/arhitektuuryolo/')
    with cd(env.code_dir + '/arhitektuuryolo'), prefix('. ../venv/bin/activate'):
    ...
    sudo('chown -R www-data:www-data /var/log/arhitektuuryolo/')

    # Copy logrotate, nginx, crontab and gunicorn confs
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/arhitektuuryolo')
        sudo('cp deploy/%s /etc/nginx/sites-enabled/arhitektuuryolo' % env.nginx_conf)
        sudo('cp deploy/gunicorn.conf /etc/init/gunicorn-arhitektuuryolo.conf')
        sudo('cp deploy/crontab.conf /etc/cron.d/arhitektuuryolo')
...
def collectstatic(npm_install=True, npm_build=True):
    with cd(env.code_dir + '/arhitektuuryolo'):
        if npm_install:
